...
def main(args):
    ...
    optimizer = torch.optim.AdamW(
        ...
        lr = config["fine_tune_params"]["learning_rate"],
        weight_decay = 4e-5
    )
    ...
    log_file_val_path = os.path.join(log_dir, f"val_log_{timestamp}.txt")
    ...
    plot_miou(epochs_train, miou_train, save_path=os.path.join(metric_dir, f"miou_per_epoch_train.png"))
    plot_loss(epochs_train, loss_train, save_path=os.path.join(metric_dir, f"loss_per_epoch_train.png"))


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Testing script for SAM2 model")
    parser.add_argument("--positive_point", type=int, default=2, help="jumlah titik positif")
    parser.add_argument("--negative_point", type=int, default=2, help="jumlah titik negatif")
